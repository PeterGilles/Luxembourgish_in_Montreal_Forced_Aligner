...
- **`dictionary/luxembourgish_mfa_run6.dict`** — Pronunciation dictionary with probabilities (trained on a Luxembourgish corpus). Use it as the dictionary in `mfa align`. Many words include **common pronunciation variants** (e.g. *déi* → [diː], *wou* → [wuː], *gehalen* → [gəhaːl], *géif* → [gif]); the trained probabilities help the aligner choose among them.
